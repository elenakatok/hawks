...
export const seedMatchTest = onRequest(async (req, res) => {
  ...
  // Hawks: 3 roles, composition { angel:1, agent:1, hawks:2 }
  type SeedP = { id: string; role: 'angel' | 'agent' | 'hawks' }
...
// Hawks group composition: 1 angel + 1 agent + 2 hawks = 4 participants.
export const seedGroupForTest = onRequest(async (req, res) => {
  ...
  const body = (req.body?.data ?? req.body) as {
    ...
    group_id?: unknown
    lead_id?: unknown
    angel_participants?: unknown
    agent_participants?: unknown
    hawks_participants?: unknown
  }
  ...
  if (
    !Array.isArray(body.angel_participants) ||
    !Array.isArray(body.agent_participants) ||
    !Array.isArray(body.hawks_participants)
  ) {
    res.status(400).json({ error: 'angel_participants, agent_participants, and hawks_participants arrays required' }); return
  }

  const gameInstanceId = body.game_instance_id
  const groupId = body.group_id
  const leadId = body.lead_id
  const angelPids = body.angel_participants as string[]
  const agentPids = body.agent_participants as string[]
  const hawksPids = body.hawks_participants as string[]
  ...
  // Write group doc with all 3 role arrays.
  const groupRef = instanceRef.collection('groups').doc(groupId)
  await groupRef.set({
    group_id: groupId,
    game_instance_id: gameInstanceId,
    angel_participants: angelPids,
    agent_participants: agentPids,
    hawks_participants: hawksPids,
