...
type GroupData = {
  ...
  // 3-role participant arrays — matching hawksGameDef composition {angel:1, agent:1, hawks:2}
  angel_participants: string[]
  agent_participants: string[]
  hawks_participants: string[]
...
function deriveRoleKey(groupData: GroupData, participantId: string): string {
  if (groupData.angel_participants.includes(participantId)) return 'angel'
  if (groupData.agent_participants.includes(participantId)) return 'agent'
  if (groupData.hawks_participants.includes(participantId)) return 'hawks'
...
export default function OutcomeReporting({
  ...
  const roleLabel = labelFor(hawksConfig, roleKey)
