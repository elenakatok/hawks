...
import { hawksConfig, hawksSchema, FIELD_LABELS, formatField } from '../gameConfig'
...
// ── Hawks outcome formatter (PLACEHOLDER — real formatter in Part 3) ──────────

function formatHawksOutcome(
  ...
  if (session.kind === 'no-token') {
    return (
      <main style={{ padding: '2rem', fontFamily: typography.fontFamily, maxWidth: '480px', margin: '2rem auto' }}>
        <h2 style={{ marginBottom: '0.75rem' }}>Hawks</h2>
        <p>Please launch Hawks from the classroom to join a session.</p>
    ...
    <div style={{ fontFamily: typography.fontFamily }}>
      ...
      {phase.name === 'off-platform' && (
        <>
          <div style={{ maxWidth: layout.contentWidth, margin: '0 auto', paddingTop: layout.pagePad, paddingLeft: layout.pagePad, paddingRight: layout.pagePad }}>
            <GroupMembersPanel groupId={phase.groupId} participantId={participantId} gameInstanceId={gameInstanceId} roleConfig={hawksConfig} db={db} rtdb={rtdb} functions={functions} />
          </div>
          <OffPlatformHolding
            groupId={phase.groupId}
            participantId={participantId}
      ...
      {phase.name === 'results' && (
        <Results
          ...
          gameInstanceId={gameInstanceId}
          roleConfig={hawksConfig}
          formatOutcome={formatHawksOutcome}
          db={db}
          rtdb={rtdb}
          functions={functions}
        />
